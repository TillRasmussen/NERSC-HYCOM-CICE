...
!!!! TILL SHOULD ENABLE_ATM STAY?
#if defined(ENABLE_ATM) || defined (ESPC_COUPLE) || defined (USE_NUOPC_CESMBETA)
!         pang
          i = index(cline,'=')
          read (cline(i+1:),*) hminb,hmaxb
          if     (mnproc.eq.1) then
...
#endif
!!!!! TILL SHOULD ENABLE_ATM STAY?
#if defined(ESPC_COUPLE) || defined (USE_NUOPC_CESMBETA) || defined(ENABLE_ATM)
